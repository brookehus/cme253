...
__global__ void cuContactsRED(double *pxyz, double *lxyz, double *cudists, int *plength, int *llength)
{
  ...
  __shared__ double temp[THREADS_PER_BLOCK_X][THREADS_PER_BLOCK_Y+1][THREADS_PER_BLOCK_Z];

  if ( (pidx < plength[0]) && (lidx< llength[0]) && ((task+1) % 4 != 0) ){

        temp[threadIdx.x][threadIdx.y][threadIdx.z] =  (pxyz[pidx*3+task]-lxyz[lidx*3+task])
                                                         *(pxyz[pidx*3+task]-lxyz[lidx*3+task]);
  }  
  __syncthreads();

  if ( (pidx < plength[0]) && (lidx< llength[0])){

    cudists[pidx+plength[0]*lidx] = sqrt(temp[threadIdx.x][threadIdx.y][0] + 
                                         temp[threadIdx.x][threadIdx.y][1] +
                                         temp[threadIdx.x][threadIdx.y][2])/10.;
   }
