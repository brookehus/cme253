...
__global__ void cuContactsRED(double *pxyz, double *lxyz, double *cudists, int *plength, int *llength)
{
  ...
  if ( (pidx < plength[0]) && (lidx< llength[0]) ){
        ...
        temp[threadIdx.x][threadIdx.y][threadIdx.z] =  (pxyz[pidx*3+task%4]-lxyz[lidx*3+task%4])
                                                         *(pxyz[pidx*3+task%4]-lxyz[lidx*3+task%4]);
  }  
  __syncthreads();

  if ( (pidx < plength[0]) && (lidx< llength[0]) && threadIdx.z % 4 == 0){

    cudists[pidx+plength[0]*lidx] = sqrt(temp[threadIdx.x][threadIdx.y][threadIdx.z] + 
                                         temp[threadIdx.x][threadIdx.y][threadIdx.z] +
                                         temp[threadIdx.x][threadIdx.y][threadIdx.z])/10.;
   }
