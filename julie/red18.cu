...
/* cuda contact featurizer */
__global__ void cuContactsRED(double *pxyz, double *lxyz, double *cudists, int *plength, int *llength)
{
  int pidx = threadIdx.x + blockIdx.x * blockDim.x;
  int lidx = threadIdx.y + blockIdx.y * blockDim.y;
  int task = threadIdx.z + blockIdx.z * blockDim.z;

  __shared__ double temp[THREADS_PER_BLOCK_X][THREADS_PER_BLOCK_Y][THREADS_PER_BLOCK_Z];

  if ( (pidx < plength[0]) && (lidx< llength[0]) ){

        temp[threadIdx.x][threadIdx.y][threadIdx.z] =  (pxyz[pidx*3+task%3]-lxyz[lidx*3+task%3])
                                                         *(pxyz[pidx*3+task%3]-lxyz[lidx*3+task%3]);
  }  
  __syncthreads();

  if ( (pidx < plength[0]) && (lidx< llength[0]) && threadIdx.z % 3  == 0){

    cudists[pidx+plength[0]*lidx] = sqrt(temp[threadIdx.x][threadIdx.y][threadIdx.z] + 
                                         temp[threadIdx.x][threadIdx.y][threadIdx.z+1] +
                                         temp[threadIdx.x][threadIdx.y][threadIdx.z+2])/10.;
   }
...
/* setup threadblock size and grid sizes*/
  ...
  cuContactsRED<<< blocks, threads >>>( d_pxyz, d_lxyz, d_cudists, d_plength, d_llength );
  checkKERNEL();
